...
- This re-execution branch is now `CLOSED` per user instruction because the
  challenge was already being pursued in `runtimeverification/mir-semantics#985`.
- The table below is retained as the final branch-local audit snapshot rather
  than an active execution plan.
...
| `unchecked_shl` | `unchecked_shl.rs`; `unchecked_shl-fail.rs`; `show/unchecked_shl-fail.*.expected` | Partial | Part 1: `i8`, `i16`, `i32`, `i64`, `i128`, `u8`, `u16`, `u32`, `u64`, `u128` | Branch-local passes: `unchecked_shl_u8`, `unchecked_shl_u16`, `unchecked_shl_u32`, `unchecked_shl_u64`, `unchecked_shl_u128`, and the last validated local checkpoint `unchecked_shl_i8`. This branch closed before widening the signed-half matrix further. |
