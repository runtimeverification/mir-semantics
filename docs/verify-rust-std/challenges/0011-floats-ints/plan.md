...
Next generator task:
- Prove `unchecked_shl_u64` end-to-end with a scoped `kmir prove-rs` run on `kmir/src/tests/integration/data/verify-rust-std/0011-floats-ints/unchecked_shl.rs` using `--start-symbol unchecked_shl_u64`, then record whether the branch extends the already-passing `unchecked_shl` family beyond `unchecked_shl_u32` without reopening the exhausted `unchecked_shr` diagnostic path.
...
2. Execute one new Part 1 proof slice, starting with `unchecked_shl_u64`, because the completed `unchecked_shr` diagnostics still collapse to the same `binOpShrUnchecked` frontier and do not justify spending another generator turn there.
