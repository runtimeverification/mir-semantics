...
Next generator task:
- Prove `unchecked_shl_u16` end-to-end with a scoped `kmir prove-rs` run on `kmir/src/tests/integration/data/verify-rust-std/0011-floats-ints/unchecked_shl.rs` using `--start-symbol unchecked_shl_u16`, then record whether the branch extends the already-passing `unchecked_shl` family beyond `unchecked_shl_u64` without reopening the exhausted `unchecked_shr` diagnostic path.
