...
| `Rc<T:?Sized,A:Allocator>::from_raw_in` | `alloc::rc` | Frontier recorded; not yet proven | Raw pointer must come from `Rc::into_raw`, allocator provenance must match, and ownership must be recovered exactly once. | Current challenge-local harness is `kmir/src/tests/integration/data/verify-rust-std/0026-rc/rc-from-raw-in-frontier-fail.rs`; the proof frontier still ends at `CastKind::Transmute`. |
