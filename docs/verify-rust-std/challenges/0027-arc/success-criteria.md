...
| `Arc<T, A>::from_raw_in` | `alloc::sync` | Proof harness added; bounded proof attempt recorded a frontier | Raw pointer must come from `Arc::into_raw`, allocator provenance must match, and ownership must be recovered exactly once. | Symbolic proof harness: `kmir/src/tests/integration/data/verify-rust-std/0027-arc/arc-from-raw-in.rs`. The bounded proof stopped at leaf `4`, where `Box::<alloc::sync::ArcInner<u32>, std::alloc::System>::new_uninit_in` reaches a `castKindTransmute` frontier. |
