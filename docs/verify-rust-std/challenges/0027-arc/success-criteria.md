...
| `Arc<T, A>::from_raw_in` | `alloc::sync` | First tranche target | Raw pointer must come from `Arc::into_raw`, allocator provenance must match, and ownership must be recovered exactly once. | Planned symbolic proof harness: `kmir/src/tests/integration/data/verify-rust-std/0027-arc/arc-from-raw-in.rs`. Temporary frontier reproducer: `kmir/src/tests/integration/data/verify-rust-std/0027-arc/arc-from-raw-in-frontier-fail.rs`. |
