...
For constant operands, there is no local place to extend with a deref projection, so we first evaluate the
constant to a pointer value and then dereference that value explicitly.
...
```k
  ...
  rule <k> #execIntrinsic(IntrinsicFunction(symbol("volatile_load")), operandConstant(CONST) .Operands, DEST, _SPAN)
        => #execVolatileLoadConstant(DEST, operandConstant(CONST))
       ... </k>

  syntax KItem ::= #execVolatileLoadConstant(Place, Evaluation) [strict(2)]
                 | #finishVolatileLoad(Place)

  // Constant operands evaluate to a pointer value, so dereference the resulting value explicitly.
  rule <k> #execVolatileLoadConstant(DEST, PTR:Value)
        => #traverseProjection(toLocal(0), PTR, projectionElemDeref .ProjectionElems, .Contexts)
        ~> #readProjection(false)
        ~> #finishVolatileLoad(DEST)
       ... </k>

  rule <k> VAL:Value ~> #finishVolatileLoad(DEST) => #setLocalValue(DEST, VAL) ... </k>
```
