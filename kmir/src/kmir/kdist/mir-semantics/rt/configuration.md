...
module KMIR-CONFIGURATION
  ...
  configuration <kmir>
                  ...
                  <stack> .List </stack>
                  // static data lookup tables (populated in concrete mode, noMap in symbolic mode)
...
holding `MaybeMap` values. In concrete mode they are populated with `someMap(M)` for efficient
LLVM execution; in symbolic mode they are `noMap` and lookups use the generated functions instead.
