...
```k
  ...
  rule <k> #traverseProjection(DEST, RangeInteger(LEN, WIDTH, SIGNED, ELEMS), .ProjectionElems, CTXTS) ~> #derefTruncate(staticSize(SIZE), PROJS)
        => #traverseProjection(DEST, RangeInteger(LEN -Int SIZE, WIDTH, SIGNED, substrBytes(ELEMS, 0, lengthBytes(ELEMS) -Int (WIDTH *Int (LEN -Int SIZE)))), PROJS, CTXTS)
        ...
       </k>
    requires 0 <=Int SIZE andBool WIDTH *Int (LEN -Int SIZE) <=Int lengthBytes(ELEMS) [preserves-definedness] // range parameters checked
