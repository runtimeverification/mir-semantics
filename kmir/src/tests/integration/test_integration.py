...
def test_verify_rust_std(rs_file: Path, kmir: KMIR, update_expected_output: bool) -> None:
    ...
    # Auto-detect start symbols: user-defined functions (no '::') with arguments
    # are proved symbolically over all inputs. Fall back to 'main' otherwise.
    start_symbols = sorted(
        name for name, args in smir_info.function_arguments.items() if args and name != 'main' and '::' not in name
    )
