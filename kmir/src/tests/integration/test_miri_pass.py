...
if not RS_FILES:
    pytestmark.append(pytest.mark.skip(reason='No imported Miri pass suite found. Run `make fetch-test-suites`.'))


def test_miri_pass_suite_matrix_alignment() -> None:
...
def test_miri_pass(rs_file: Path, kmir: KMIR) -> None:
