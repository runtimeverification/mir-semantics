...
# they don't differ between local checkouts and CI (e.g. symbolic-args-unsupported.main.cli-stats-leaves).
...
def test_cli_show_printers_snapshot(
    ...
    assert_or_update_show_output(
        out_custom,
        PROVE_DIR / f'show/{rs_file.stem}.{start_symbol}.cli-custom-printer.expected',
    ...
    assert_or_update_show_output(
        out_default,
        PROVE_DIR / f'show/{rs_file.stem}.{start_symbol}.cli-default-printer.expected',
        update=update_expected_output,
    )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            (PROVE_DIR / 'symbolic-args-unsupported.rs'),
            'main',
            False,
            id='symbolic-args-unsupported.main',
        ),
        pytest.param(
...
def test_cli_info_snapshot(capsys: pytest.CaptureFixture[str], update_expected_output: bool) -> None:
    smir_json = PROVE_DIR / 'arith.smir.json'
    ...
    out = capsys.readouterr().out.rstrip()
    assert_or_update_show_output(
        out, PROVE_DIR / f'show/{smir_json.stem}.cli-info.expected', update=update_expected_output
    )


def test_cli_link_counts_snapshot(tmp_path: Path, update_expected_output: bool) -> None:
    smir1 = PROVE_DIR / 'arith.smir.json'
    ...
    assert_or_update_show_output(
        counts_text,
        PROVE_DIR / f'show/link.{smir1.stem}+{smir2.stem}.counts.expected',
        update=update_expected_output,
    )
...
def test_cli_prune_snapshot(
    ...
    out = capsys.readouterr().out.rstrip()
    assert_or_update_show_output(
        out, PROVE_DIR / f'show/{rs_file.stem}.{start_symbol}.cli-prune.expected', update=update_expected_output
    )
...
def test_cli_show_to_module(
    ...
    assert_or_update_show_output(
        k_content.rstrip(),
        PROVE_DIR / f'show/{rs_file.stem}.{start_symbol}.to-module.k',
        update=update_expected_output,
    )
    ...
    assert_or_update_show_output(
        json_content.rstrip(),
        PROVE_DIR / f'show/{rs_file.stem}.{start_symbol}.to-module.json',
        update=update_expected_output,
    )
...
def test_cli_show_minimize_proof(kmir: KMIR, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    rs_file = PROVE_DIR / 'symbolic-args-unsupported.rs'
    start_symbol = 'main'
...
def test_cli_prove_add_module(kmir: KMIR, tmp_path: Path) -> None:
    """Test --add-module option for prove using stored module files."""
    from kmir.kmir import KMIR
    ...
    stored_module_json = PROVE_DIR / f'show/{rs_file.stem}.{start_symbol}.to-module.json'
    assert stored_module_json.exists(), f'Stored JSON module file not found: {stored_module_json}'

    # Run prove with --add-module and max_depth=1
...
def test_cli_break_on_function(
    ...
    rs_file = PROVE_DIR / 'break-on-function.rs'
    start_symbol = 'main'
