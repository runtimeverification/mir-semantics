#!/usr/bin/env python3
"""
Script to verify the add1 function from cse_call_add1_Ntime.rs using KMIR.
Supports different iteration counts (1time, 2time, etc.).
...
def verify_add1(times=1, start_symbol='add1'):
    """
    Verify the add1 function with specified iteration count and return the APRProof object.
    Also saves the proof show output to the proof directory.
    
    Args:
        times: Number of iterations (1, 2, 3, etc.)
        start_symbol: Function to use as entry point (default: 'add1')
    """
    # Path to the SMIR JSON file (pre-generated) based on times parameter
    smir_file = Path(f'/Users/steven/Desktop/projs/mir-semantics/kmir/src/tests/integration/data/exec-smir/cse/cse_call_add1_{times}time.smir.json')
    ...
    # Set up proof directory with times suffix
    proof_dir = Path(f'./add1_{times}time_proof')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Verify add1 function with specified iteration count')
    parser.add_argument('times', type=int, nargs='?', default=1,
                        help='Number of iterations (default: 1)')
    parser.add_argument('--start-symbol', type=str, default='add1',
                        help='Function to use as entry point (default: add1)')
    
    args = parser.parse_args()
    
    try:
        proof = verify_add1(times=args.times, start_symbol=args.start_symbol)
